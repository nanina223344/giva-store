...
const SIDEBAR_NAV = [
  {
    ...
    items: [
      ...
      { href: '/admin/articles.html',   label: 'Inspirasi',       icon: 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z' },
    ],
  },
  {
    group: 'Pelanggan',
  ...
  },
  {
    ...
    items: [
      ...
      {
        label:  'Stok',
        icon:   'M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8l1 12a2 2 0 002 2h8a2 2 0 002-2l1-12M10 12v5m4-5v5',
        parent: true,
        id:     'sidebar-stok-parent',
        children: [
          { href: '/admin/stock.html',        label: 'Manajemen Stok' },
          { href: '/admin/stock-zero.html',   label: 'Stok Nol' },
          { href: '/admin/stock-history.html',label: 'Riwayat Koreksi' },
        ],
      },
    ],
  },
  {
    group: 'Sistem',
    items: [
      { href: '/admin/expenses.html',  label: 'Keuangan',         icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
      ...
      { href: '/kasir/index.html',     label: 'Kasir (POS)',      icon: 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h.01M12 11h.01M15 11h.01M4 7h16a1 1 0 011 1v10a1 1 0 01-1 1H4a1 1 0 01-1-1V8a1 1 0 011-1z', target: '_blank', noActive: true },
...
function buildNavItem(item, currentPath) {
  ...
  const badgeHTML = item.badgeId
    ? `<span id="${item.badgeId}" class="ml-auto hidden px-2 py-0.5 rounded-full bg-red-500 text-white text-[10px] font-bold leading-none shadow-sm"></span>`
    : ''
...
/**
 * Render parent menu dengan toggle expand/collapse dan submenu children.
 */
function buildParentItem(item, currentPath) {
  // Auto-expand jika salah satu child aktif
  const hasActiveChild = item.children.some(child =>
    currentPath.endsWith(child.href.split('/').pop())
  )
  const expanded = hasActiveChild

  const parentBtnClass = hasActiveChild
    ? 'text-sage-deep bg-sage/10 border-sage/25 font-semibold'
    : 'text-stone hover:text-ink hover:bg-cream border-transparent'
  const iconColor = hasActiveChild ? 'color: var(--color-sage)' : ''
  const chevronClass = expanded ? 'rotate-90' : ''

  const childrenHTML = item.children.map(child => {
    const isActive = currentPath.endsWith(child.href.split('/').pop())
    const childClass = isActive
      ? 'text-sage-deep bg-sage/10 font-semibold'
      : 'text-stone hover:text-ink hover:bg-cream'
    const dotClass = isActive ? 'bg-sage' : 'bg-stone/25 group-hover/child:bg-stone/50'
    return `
      <a href="${child.href}"
        class="group/child flex items-center gap-2.5 pl-9 pr-3 py-2 rounded-lg text-sm font-medium transition-all ${childClass}">
        <span class="w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${dotClass}"></span>
  ...
  return `
    <div>
      <button data-sidebar-toggle="${item.id}"
        class="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium border transition-all ${parentBtnClass}">
        <svg class="w-4 h-4 flex-shrink-0" style="${iconColor}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${item.icon}" />
        </svg>
        <span class="flex-1 text-left">${item.label}</span>
        <svg id="${item.id}-chevron"
          class="w-3.5 h-3.5 flex-shrink-0 transition-transform duration-200 ${chevronClass}"
...
function buildSidebar(staffUser) {
  ...
  const navHTML = SIDEBAR_NAV.map(group => `
    ...
    ${group.items.map(item =>
      item.parent
        ? buildParentItem(item, currentPath)
        : buildNavItem(item, currentPath)
    ).join('')}
  `).join('')
...
/**
 * Wire toggle events untuk semua parent-menu buttons.
 * Dipanggil sekali setelah sidebar di-inject ke DOM.
 */
 ...
 * @param {function} logoutFn — fungsi logout halaman (opsional)
 */
...
export function initSidebar(staffUser, logoutFn) {
  ...
  // Wire collapse/expand toggle buttons untuk parent menu
  wireSidebarToggles()
